...
The T2 is an exception and should be modified only in `double _Complex *test_f(...)`.
This parameter is defined around line 870 of the source code.
...
double _Complex *test_f ( double t, int m, double _Complex u[], double tau, double kx, double ky, double alpha ){
    ...
    double pi = 3.14159265358979323846;
    double T1 = 999999999.0;
    double T2 = 826.8289*1.0;// dephasing time
...
// 
    ...
    gsl_matrix_complex *U_T = NULL;
    gsl_matrix_complex *COMM1 = NULL;
    gsl_matrix_complex *COMM2 = NULL;

    gsl_matrix_complex *RHO_T_TILDE = NULL;
    gsl_matrix_complex *RHO_T_TILDE2 = NULL;
    gsl_matrix_complex *MIDDLE2 = NULL;
    gsl_matrix_complex *MIDDLE3 = NULL;
    gsl_matrix_complex *D = NULL;

    H_T = gsl_matrix_complex_alloc(size2, size2);
    RHO_T = gsl_matrix_complex_alloc(size2, size2);
    U_T = gsl_matrix_complex_alloc(size2,size2);
    COMM1 = gsl_matrix_complex_alloc(size2, size2);
    COMM2 = gsl_matrix_complex_alloc(size2, size2);

    RHO_T_TILDE = gsl_matrix_complex_alloc(size2, size2);
    RHO_T_TILDE2 = gsl_matrix_complex_alloc(size2,size2);
    MIDDLE2 = gsl_matrix_complex_alloc(size2, size2);
    MIDDLE3 = gsl_matrix_complex_alloc(size2, size2);
    ...
    gsl_matrix_complex_set_all(RHO_T_TILDE, GSL_COMPLEX_ZERO);
    gsl_matrix_complex_set_all(RHO_T_TILDE2, GSL_COMPLEX_ZERO);
    gsl_matrix_complex_set_all(MIDDLE2, GSL_COMPLEX_ZERO);
    gsl_matrix_complex_set_all(MIDDLE3, GSL_COMPLEX_ZERO);
    gsl_matrix_complex_set_all(D, GSL_COMPLEX_ZERO);

    info2 = LAPACKE_zheev(LAPACK_ROW_MAJOR, 'V', 'U', n2, H_KA, lda2, E_eigval2);
    if(info2 !=0){printf("LAPACKE_zheev error %d\n",info2); }
...
// 
    ...
    for ( rhi = 0 ; rhi < size2 ; rhi++ ) {
        for ( rhj = 0 ; rhj < size2 ; rhj++ ) {
            gsl_matrix_complex_set(U_T, rhi, rhj, gsl_complex_rect(creal(H_KA[rhi * size2 + rhj]),cimag(H_KA[rhi * size2 + rhj])));
        }
    }

    gsl_blas_zgemm(CblasConjTrans, CblasNoTrans, GSL_COMPLEX_ONE, U_T, RHO_T,
                    GSL_COMPLEX_ZERO, MIDDLE2);

    gsl_blas_zgemm(CblasNoTrans, CblasNoTrans, GSL_COMPLEX_ONE, MIDDLE2, U_T,
                    GSL_COMPLEX_ZERO, RHO_T_TILDE);
    ...
    for ( rhi = 0 ; rhi < size2 ; rhi++ ) {
        for ( rhj = 0 ; rhj < size2 ; rhj++ ) {
            ...
            gsl_matrix_complex_set(RHO_T_TILDE2, rhi, rhj, gsl_complex_rect( GSL_REAL(gsl_matrix_complex_get(RHO_T_TILDE,rhi,rhj))/T2,GSL_IMAG(gsl_matrix_complex_get(RHO_T_TILDE,rhi,rhj))/T2 ));
    ...
    for ( rhi = 0 ; rhi < size2 ; rhi++ ) {
        ...
        gsl_matrix_complex_set(RHO_T_TILDE2, rhi, rhi, gsl_complex_rect( (GSL_REAL(gsl_matrix_complex_get(RHO_T_TILDE,rhi,rhi)) - 0.0)/T1,GSL_IMAG(gsl_matrix_complex_get(RHO_T_TILDE,rhi,rhi))/T1 ));

    }

    gsl_matrix_complex_set(RHO_T_TILDE2, 0, 0, gsl_complex_rect( (GSL_REAL(gsl_matrix_complex_get(RHO_T_TILDE,0,0)) - 1.0)/T1,GSL_IMAG(gsl_matrix_complex_get(RHO_T_TILDE,0,0))/T1 ));

    gsl_blas_zgemm(CblasNoTrans, CblasNoTrans, GSL_COMPLEX_ONE, U_T, RHO_T_TILDE2,
                    GSL_COMPLEX_ZERO, MIDDLE3);

    gsl_blas_zgemm(CblasNoTrans, CblasConjTrans, GSL_COMPLEX_ONE, MIDDLE3, U_T,
                    GSL_COMPLEX_ZERO, D);
    ...
    for ( oparai = 0 ; oparai < size2 ; oparai++ ) {
        for ( oparaj = 0 ; oparaj < size2 ; oparaj++ ) {
            arr[oparai * size2 + oparaj] = -1.0*I * ( GSL_REAL(gsl_matrix_complex_get(COMM1,oparai,oparaj)) + GSL_IMAG(gsl_matrix_complex_get(COMM1,oparai,oparaj))*I - GSL_REAL(gsl_matrix_complex_get(COMM1,oparaj,oparai)) + GSL_IMAG(gsl_matrix_complex_get(COMM1,oparaj,oparai))*I )
                                                - ( GSL_REAL(gsl_matrix_complex_get(D,oparai,oparaj)) + GSL_IMAG(gsl_matrix_complex_get(D,oparai,oparaj))*I );
        }
    }
    ...
    gsl_matrix_complex_free(U_T);
    gsl_matrix_complex_free(COMM1);
    gsl_matrix_complex_free(COMM2);

    gsl_matrix_complex_free(RHO_T_TILDE);
    gsl_matrix_complex_free(RHO_T_TILDE2);
    gsl_matrix_complex_free(MIDDLE2);
    gsl_matrix_complex_free(MIDDLE3);
